...
class rnn(TFBaseModel):
    ...
    def calculate_outputs(self, x):
        ...
        final_temporal_idx = tf.stack([tf.range(tf.shape(self.history_length)[0]), tf.maximum(self.history_length - 1, 0)], axis=1) # Add tf.maximum to handle zero length problem
